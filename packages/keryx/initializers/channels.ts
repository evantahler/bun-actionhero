...
export class Channels extends Initializer {
  ...
  async initialize() {
    ...
    // Channels are user-defined only (no framework channels)
    const userChannelDir = path.join(api.rootDir, config.paths.channels);
    if (existsSync(userChannelDir)) {
      try {
        channels = await globLoader<Channel>(userChannelDir);
      } catch (e) {
        logger.debug(
